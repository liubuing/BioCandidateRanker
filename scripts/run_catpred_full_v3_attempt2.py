...
def main():
    ...
    protocol_path=ROOT/'configs/catpred_mode_b_protocol_v3_attempt2.json'
    ...
    print('Verifying cached development features', flush=True)
    for index, entry in enumerate(manifest['features'], 1):
        assert sha(ROOT/entry['cache_path'])==entry['sha256'],entry['cache_path']
        if index % 1000 == 0: print(f'Verified {index}/{len(manifest["features"])}', flush=True)
    target=BASE/'full-v3-attempt2'/f'seed{args.seed}'
    ...
    command=[sys.executable,'-u',str(ROOT/'scripts/catpred_memory_entry.py'),
